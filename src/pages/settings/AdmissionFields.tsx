...
const RegistrationFieldsSettings: React.FC = () => {
    ...
    const handleSave = async () => {
        ...
        try {
            // Use school-specific path
            const path = `schools/${schoolId}/settings/registration_fields`;
            alert('About to save to: ' + path);
            console.log('[AdmissionFields] Saving to path:', path);
            console.log('[AdmissionFields] Saving', fields.length, 'fields');
            const docRef = doc(db, path);
            await setDoc(docRef, {
                fields: fields,
                updatedAt: new Date().toISOString()
            });
            console.log('[AdmissionFields] Settings saved successfully!');
