...
const EnhancedExamScheduling: React.FC = () => {
    ...
    const schoolClasses = sortClasses(settings?.filter(s =>
        s.schoolId === currentSchool?.id &&
        s.type === 'class' &&
        s.active !== false &&
        (!s.financialYear || s.financialYear === activeYear?.name)
    ) || []);
    ...
    // ─── Recovery Tool: Only recover from marks_entries ───────────
    const handleRecoverLostClasses = async (examId: string) => {
        if (!confirm('Recover classes and subjects from marks entries only?')) return;

        try {
            const exam = schoolExams.find(e => e.id === examId);
            if (!exam) { alert('Exam not found'); return; }

            // Use own subjects[] for correct maxMarks/dates
            const ownSubjectsMap = new Map<string, any>();
            (exam.subjects || []).forEach((s: any) => {
                if (s.subjectId) ownSubjectsMap.set(s.subjectId, s);
                if (s.subjectName) ownSubjectsMap.set('name_' + s.subjectName.toLowerCase().replace(/[^a-z0-9]/g, ''), s);
            });
            ...
            examMarks.forEach((m: any) => {
                if (!m.classId || !m.subjectId) return;
                classIds.add(m.classId);

                if (!crMap.has(m.classId)) {
                    crMap.set(m.classId, { classId: m.classId, className: m.className || resolveClassName(m.classId), routine: [] });
                }
                const cr = crMap.get(m.classId)!;
                if (!cr.routine.some(r => r.subjectId === m.subjectId)) {
                    const own = ownSubjectsMap.get(m.subjectId) || ownSubjectsMap.get('name_' + (m.subjectName || '').toLowerCase().replace(/[^a-z0-9]/g, ''));
                    const entry: any = {
                        subjectId: m.subjectId,
                        subjectName: m.subjectName || m.subjectId,
                        assessmentType: own?.assessmentType || 'MARKS',
                        maxMarks: own?.maxMarks || m.maxMarks || defMax,
                        passingMarks: own?.passingMarks || 33,
                        examDate: own?.examDate || exam.startDate || '',
                        examTime: own?.examTime || '09:00',
                        duration: own?.duration || 180
                    };
                    if (own?.theoryMarks !== undefined) entry.theoryMarks = own.theoryMarks;
                    if (own?.practicalMarks !== undefined) entry.practicalMarks = own.practicalMarks;
                    if (own?.combinedSubjects) entry.combinedSubjects = own.combinedSubjects;
                    cr.routine.push(entry);
                }
            });
            ...
            const classOrder = (name: string): number => {
                ...
                if (n.includes('pre-nursery') || n.includes('prenursery') || n.includes('pre nursery')) return 0;
                if (n === 'nursery') return 1;
                if (n === 'lkg') return 2;
                if (n === 'ukg') return 3;
                const match = n.match(/class\s*(\d+)/i);
                if (match) return 4 + parseInt(match[1]);
                return 100;
            };

            const newClassRoutines = Array.from(crMap.values())
                .sort((a, b) => classOrder(a.className) - classOrder(b.className));
            const newTargetClasses = newClassRoutines.map(cr => cr.classId);

            console.log('Recovered', newTargetClasses.length, 'classes from marks (sorted):');
            newClassRoutines.forEach(cr => console.log(' ', cr.className, ':', cr.routine.map(r => r.subjectName + '=' + r.maxMarks).join(', ')));

            await updateDocument(examId, { targetClasses: newTargetClasses, classRoutines: newClassRoutines, updatedAt: new Date().toISOString() });
            alert('Done! Recovered ' + newTargetClasses.length + ' classes from marks entries.');
        } catch (error) {
            console.error('Recovery error:', error);
            alert('Recovery failed: ' + (error as any)?.message);
        }
    };
    ...
    const getClassRoutine = (classId: string): ClassRoutine => {
        return newExam.classRoutines?.find(cr =>
            cr.classId === classId ||
            cr.className === resolveClassName(classId) ||
            resolveClassName(cr.classId) === resolveClassName(classId)
        ) || {
            classId,
    ...
    const updateClassRoutine = (classId: string, updatedRoutineOrFn: ClassRoutineEntry[] | ((prevRoutine: ClassRoutineEntry[]) => ClassRoutineEntry[])) => {
        setNewExam(prev => {
            ...
            const idx = existing.findIndex(cr =>
                cr.classId === classId ||
                cr.className === resolveClassName(classId) ||
                resolveClassName(cr.classId) === resolveClassName(classId)
            );
            ...
            if (idx >= 0) {
                updated[idx] = { ...updated[idx], classId, routine: finalRoutine }; // Keep the newest classId
            } else {
    ...
    const handleDeleteExam = async (id: string) => {
        if (!confirm('Are you sure you want to cancel this exam? This action cannot be undone.')) return;
        try {
    ...
    return (
        <div className="page-container">
            <div className="page-header" style={{
                ...
                {/* TODO: Remove these 4 utility/debug buttons in a future cleanup:
                    deepRecoverLostClasses, fixYearSuffixDiscrepancy, exportGlobalSlots, uploadSchedule
                */}
                {/* HIDDEN UTILITY BUTTONS - set false to true to re-enable */}
                {false && (<>
                    <button
                        onClick={async () => {
                            try {
                                const firstTerminal = schoolExams.find(e => e.name.toLowerCase().includes('1st') || e.name.toLowerCase().includes('first'));
                                const secondTerminal = schoolExams.find(e => e.name.toLowerCase().includes('2nd') || e.name.toLowerCase().includes('second'));

                                if (!firstTerminal && !secondTerminal) {
                                    alert("Couldn't find 1st or 2nd terminal exams to recover.");
                                    return;
                                }

                                alert(`Tracking down routines inside exams itself...`);

                                const performRecovery = async (exam: any) => {
                                    if (!exam) return;

                                    const classRoutines = exam.classRoutines || [];
                                    ...
                                    classRoutines.forEach((cr: any) => {
                                        if (cr.classId) {
                                            classIds.add(cr.classId);
                                        }
                                    });

                                    if (classIds.size > 0 && Array.isArray(exam.targetClasses) && exam.targetClasses.length === 0) {
                                        const classes = Array.from(classIds);
                                        await updateDocument(exam.id, {
                                            targetClasses: classes,
                                            updatedAt: new Date().toISOString()
                                        });
                                        alert(`SUCCESS: Restored ${classes.length} mapped classes for ${exam.name}!`);
                                    }
                                };

                                await performRecovery(firstTerminal);
                                await performRecovery(secondTerminal);
                                alert("Recovery operation finished! Please refresh the page.");
                    ...
                    >
                        <Search size={20} />
                        Deep Recover Lost Classes
                    </button>
                    <button
                        onClick={async () => {
                            const targetYearSuffix = activeYear?.name.replace(/[^a-z0-9]/g, '_') || '';
                            if (!targetYearSuffix) return alert("Active academic year not found.");

                            let fixedCount = 0;
                            for (const exam of schoolExams) {
                                let updated = false;
                                const newTargetClasses = (exam.targetClasses || []).map((id: string) => {
                                    if (id.includes('_2026_2027') && targetYearSuffix === '2025_2026') {
                                        updated = true;
                                        return id.replace('_2026_2027', '_2025_2026');
                                    }
                                    return id;
                                });

                                const newClassRoutines = (exam.classRoutines || []).map((r: any) => {
                                    if (r.classId && r.classId.includes('_2026_2027') && targetYearSuffix === '2025_2026') {
                                        updated = true;
                                        return { ...r, classId: r.classId.replace('_2026_2027', '_2025_2026') };
                                    }
                                    return r;
                                });
                                ...
                                if (updated) {
                                    await updateDocument(exam.id, {
                                        ...
                                        classRoutines: newClassRoutines,
                                        updatedAt: new Date().toISOString()
                                    });
                                    fixedCount++;
                                }
                            }
                            alert(`Fixed ${fixedCount} exams! Corrected 2026_2027 suffixes to ${targetYearSuffix}.`);
                        }}
                        style={{
                            display: 'inline-flex',
                            alignItems: 'center',
                    ...
                    >
                        <CheckCircle size={20} />
                        Fix Year Suffix Discrepancy (2026→2025)
                    </button>
                    {/* EXPORT EXAM SLOTS TO EXCEL - AGGRESSIVE VERSION */}
                    <button
                        onClick={async () => {
                            try {
                                ...
                                alert("Fetching ALL available slots from the entire database (this might take a moment)...");

                                const slotsRef = collection(db, 'exam_slots');
                                // NO schoolId FILTER for deep debugging
                                const slotsSnap = await getDocs(slotsRef);

                                if (slotsSnap.empty) {
                                    alert("No slots found anywhere!");
                                    return;
                                }

                                const data = slotsSnap.docs.map(doc => {
                                    const d = doc.data();
                                    return {
                                        "School ID": d.schoolId || '(empty)',
                                        "Exam name": d.examName || '',
                                        "Exam ID": d.examId || '',
                                        "Class": d.class || '',
                                        "Class Resolved": resolveClassName(d.class) || '',
                                        "Subject": d.subject || '',
                                        "Date": d.date || '',
                                        "Time": d.startTime || '',
                                        "Slot ID": d.id || doc.id
                                ...
                                alert(`Found ${data.length} total slots. Exporting to identify your lost ones...`);

                                const worksheet = XLSX.utils.json_to_sheet(data);
                                const workbook = XLSX.utils.book_new();
                                XLSX.utils.book_append_sheet(workbook, worksheet, "All Slots (Global)");

                                XLSX.writeFile(workbook, `GLOBAL_EXAM_SLOTS_BACKUP.xlsx`);
                    ...
                    >
                        <FileText size={20} />
                        Export GLOBAL Slots (Deep Search)
                    </button>
                            ...
                            try {
                                ...
                                reader.onload = async (evt) => {
                                    try {
                                        ...
                                        alert(`Reading ${data.length} schedule entries...`);

                                        const examDataMap: Record<string, any> = {};

                                        data.forEach(row => {
                                            // Case-insensitive column lookups for better robustness
                                            ...
                                            const examId = getVal(["Exam ID", "examId", "exam_id"]);
                                            const classId = getVal(["Class ID", "classId", "class_id", "Class"]);
                                            const subject = getVal(["Subject", "subjectName", "subject_name"]);
                                            let dateRaw = getVal(["Date", "examDate", "exam_date"]);
                                            const startTime = getVal(["Start Time", "Time", "startTime", "start_time"]);
                                            const duration = getVal(["Duration (min)", "duration", "Duration"]);
                                            const maxMarks = getVal(["Max Marks", "maxMarks", "max_marks"]);
                                            ...
                                            if (dateRaw instanceof Date) {
                                                ...
                                                // Handle Excel date serial number (e.g. 45678)
                                                // Adjusted for Excel's 1900 leap year bug if needed, but simple version:
                                                const date = new Date((dateRaw - (25567 + 1)) * 86400 * 1000);
                                                formattedDate = date.toISOString().split('T')[0];
                                            } else if (typeof dateRaw === 'string') {
                                                // If it's a string, try to normalize it
                                                if (dateRaw.includes('-')) formattedDate = dateRaw; // already YYYY-MM-DD
                                                else if (dateRaw.includes('/')) {
                                                    const parts = dateRaw.split('/');
                                                    if (parts[0].length === 4) formattedDate = dateRaw.replace(/\//g, '-');
                                                    else if (parts.length === 3) formattedDate = `${parts[2]}-${parts[1].padStart(2, '0')}-${parts[0].padStart(2, '0')}`;
                                                }
                                            }

                                            if (!examDataMap[examId]) {
                                                examDataMap[examId] = { classRoutines: {} };
                                            }

                                            if (!examDataMap[examId].classRoutines[classId]) {
                                                examDataMap[examId].classRoutines[classId] = {
                                                    classId,
                                                    className: getVal(["Class Name", "className"]) || resolveClassName(classId),
                                                    routine: []
                                                };
                                            }

                                            examDataMap[examId].classRoutines[classId].routine.push({
                                                subjectId: String(subject || ''),
                                                subjectName: String(subject || ''),
                                                examDate: formattedDate,
                                                examTime: String(startTime || ''),
                                                duration: Number(duration || 120),
                                                maxMarks: Number(maxMarks || 100),
                                                assessmentType: 'MARKS',
                                                passingMarks: Math.floor(Number(maxMarks || 100) * 0.33)
                                            });
                                        });

                                        const examIds = Object.keys(examDataMap);
                                        let successCount = 0;
                                        for (const eid of examIds) {
                                            const routinesObj = examDataMap[eid].classRoutines;
                                            const finalRoutines = Object.values(routinesObj);
                                            const finalTargetClasses = finalRoutines.map((r: any) => r.classId);

                                            await updateDocument(eid, {
                                                targetClasses: finalTargetClasses,
                                                classRoutines: finalRoutines,
                                                updatedAt: new Date().toISOString()
                                            });
                                            successCount++;
                                        }

                                        alert(`Successfully restored schedule for ${successCount} exams with dates! Please refresh.`);
                                    } catch (err: any) {
                                    ...
                                    <div className="animate-fade-in">
                                        ...
                                        <div style={{
                                            ...
                                            {(() => {
                                                ...
                                                const rawIds = newExam.targetClasses || [];
                                                const allClassIds = sortClasses(rawIds.map(id => ({ id, name: resolveClassName(id) }))).map((c: any) => c.id);
